...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE products 
                              SET title = ?, description = ?, price = ?, category_id = ?, image = ?, contact_phone = ?, updated_at = NOW() 
                              WHERE id = ? AND user_id = ?");
        if ($stmt->execute([$title, $description, $price, $category_id, $image, $contact_phone, $product_id, $_SESSION['user_id']])) {
